...
#-----[UserAgent]-----
# URL of the GitHub raw TXT file containing user agents
github_user_agents_url = "https://raw.githubusercontent.com/Dragon-Lord-404/urban-tribble/main/useragent.txt"

# Create an empty list to store user agents in the desired format
ugen = []

try:
    # Make an HTTP GET request to fetch the user agents
    response = requests.get(github_user_agents_url)
    
    # Check if the request was successful
    if response.status_code == 200:
        # Extract user agents from the response content and format them
        user_agents = response.text.split('\n')
        formatted_user_agents = [f"'{ua}'" for ua in user_agents]
        
        # Extend the ugen list with the formatted user agents, joining them with newlines
        ugen.extend(formatted_user_agents)
    else:
        print("Failed to fetch user agents from GitHub.")
